...
import grupo5.criticadigital.dto.AsistenciaDTO;
...
public class AsistenciaRestController {
    ...
    public ResponseEntity<AsistenciaDTO> asistenciaDiputado(@PathVariable("id") Integer id) {
        AsistenciaDTO asistenciaDiputado = AsistenciaDTO.builder().
                NombreDiputado(diputadoService.diputadoPorId(Long.valueOf(id)).getNombres())
                .Asistencias(asistenciaService.getAsistenciadiputadoById(id))
                .Inasistencia(asistenciaService.getInasistenciaDiputadoById(id))
                .build();
        return new ResponseEntity<>(asistenciaDiputado, HttpStatus.OK);
    }
}
